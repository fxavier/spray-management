...
import { ArrowLeft, Save, AlertCircle, Calendar, MapPin, Users, Activity, Sparkles, Home, Target, Droplets } from 'lucide-react'
...
export default function NewSprayTotalPage() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [sprayersRes, communitiesRes, sprayConfigsRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/spray-totals', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        router.push('/spray-totals?success=created')
      } else {
        ...
        if (response.status === 409) {
          ...
          setErrors({ submit: data.error || 'Erro ao criar registro' })
        }
      }
    } catch (error) {
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-8">
                    ...
                    <p className="mt-3 text-xl text-slate-600 font-medium">
                      Crie um novo registro de pulverização inteligente
                    </p>
                  </div>
